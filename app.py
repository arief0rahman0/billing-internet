...
def init_db():
    ...
    if not conn.execute("SELECT id FROM admin_user WHERE username = ?", ("admin",)).fetchone():
        conn.execute(
    ...
    if not conn.execute("SELECT id FROM admin_user WHERE username = ?", ("operator",)).fetchone():
        conn.execute(
            "INSERT INTO admin_user (username, password, role) VALUES (?, ?, ?)",
...
def generate_tagihan_otomatis():
    ...
    semua_pelanggan = conn.execute("SELECT id, tagihan_bulanan FROM pelanggan").fetchall()
...
def index():
    ...
    if search_query:
        data_pembayaran = conn.execute(
            base_query + " WHERE pelanggan.nama LIKE ? ORDER BY pembayaran.bulan_tagihan DESC, pembayaran.id DESC",
            (f"%{search_query}%",),
        ).fetchall()
    else:
        data_pembayaran = conn.execute(
...
def setup_2fa():
    ...
    if request.method == "POST":
        ...
        if totp.verify(kode_verifikasi):
            ...
            conn.close()
            flash("Kode verifikasi salah! Pastikan Anda memasukkan kode yang tepat dari aplikasi.", "error")
            return redirect(url_for("setup_2fa"))
