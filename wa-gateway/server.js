    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            if (shouldReconnect) {
                ...
                console.log('Sesi tidak valid (Logged Out). Restart service untuk barcode baru...');
...
app.post('/send', async (req, res) => {
    ...
    if (!target || !message) return res.status(400).json({ status: false, error: 'Parameter kurang' });
